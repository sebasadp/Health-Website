...
import { Avatar, Badge, Card, SectionTitle, CalorieRing } from './ui'
    ...
    <div style={{ padding: 16 }}>
      ...
      {latestWeights.length > 0 && (
        <Card style={{ margin: '12px 16px' }}>
          <SectionTitle icon="⚖️">Latest weights</SectionTitle>
      ...
      <Card style={{ margin: '12px 16px' }}>
        <SectionTitle icon="💪">Today's workouts</SectionTitle>
          ...
          {data.members.map(mem => {
            ...
            return (
              <div key={mem.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                {wo ? <Badge color="green"><i className="ti ti-check" /> Done</Badge> : <Badge color="red">Pending</Badge>}
              </div>
            )
          })}
        </div>
      ...
      {/* Today's Meals - THIRD */}
      <Card style={{ margin: '12px 16px' }}>
        <SectionTitle icon="🍽️">Today's meals</SectionTitle>
                ...
                {MEAL_TYPES.map(mt => {
                  ...
                  return (
                    <div key={mt} style={{ padding: '6px 2px' }}>
                      {done
                        ? <span style={{ display: 'inline-flex', width: 18, height: 18, borderRadius: '50%', background: 'var(--green)', color: '#fff', alignItems: 'center', justifyContent: 'center', fontSize: 11 }}>✓</span>
        ...
        {data.members.some(mem => todayMeals.filter(m => m.member_name === mem.name).some(m => m.calories)) && (
          <div style={{ marginTop: 14, borderTop: '0.5px solid var(--border)', paddingTop: 14 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 10 }}>🔥 Today's calories</div>
            {data.members.map(mem => {
              ...
              const color = getMemberColor(mem.name, data.members)
              return (
                <div key={mem.id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  <CalorieRing value={cals} max={maintenance} color={color} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>{mem.name.split(' ')[0]}</div>
                    <div style={{ fontSize: 12, color: 'var(--text2)' }}>{cals} / {maintenance} cal</div>
                  </div>
                  <div style={{ fontSize: 12, color: diff > 0 ? 'var(--red)' : 'var(--green)', fontWeight: 600 }}>
                    {diff > 0 ? '+' : ''}{diff}
                  </div>
                </div>
              )
